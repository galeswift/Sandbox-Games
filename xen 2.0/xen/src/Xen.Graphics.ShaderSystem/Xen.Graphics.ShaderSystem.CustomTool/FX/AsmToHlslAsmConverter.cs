...
{
	/*
	 * Compiling an Xbox ASM shader quite often results in buggy constant use, and other 
	 * strange, unexpected results. 
	 * This is because ShaderCompiler.AssembleFromSource is buggy when targetting the xbox.
	 * 
	 * This class converts ASM shaders into HLSL shaders, which then will compile correctly.
	 * The ASM shader is then wrapped in a HLSL shader, something like:
	 * 
	 * void Shader(...)
	 * {
	 *		asm
	 ...
	 * and due to various limitations in XNA, the source for the shaders is only
	 * available in PC ASM.
	 * 
	 * So yes, technically the process is:
	 * HLSL Effect -> ASM Shader -> HLSL Shader with ASM -> Binary shader 
	 * 
	 * Warning: This code is rather old and rusty...
	 */
	public class AsmToHlslAsmConverter
	{
		...
		private Dictionary<int, bool> assignedConstants;

		private List<InputOutput> samplers;
		...
		static ShaderProfile[] VSprofileValues = new ShaderProfile[] { ShaderProfile.VS_1_1, ShaderProfile.VS_2_0, ShaderProfile.VS_2_A, ShaderProfile.VS_3_0 };
		static ShaderProfile[] PSprofileValues = new ShaderProfile[] { ShaderProfile.PS_2_0, ShaderProfile.PS_2_A, ShaderProfile.PS_2_B, ShaderProfile.PS_3_0 };
		...
		{
			if (sampler[0].Length > 0 && sampler[0][0] == '_')
				return this.samplers[int.Parse(sampler[0].Substring(2))].mapping;
			return this.samplers[int.Parse(sampler[0].Substring(1))].mapping;
		}
