...
float3	UseAlbedoOcclusionShadow		: GLOBAL; // if these values are 1, then albedo/occlusion/shadow is used
...
float4 CharacterPS(	float2 texCoordIn		: TEXCOORD0,
					...
					float4 shadowMapSample	: TEXCOORD5) : COLOR 
{
	...
	//convert the rgb colour into linear, this could be done offline
	sunColour			*= sunColour;
	...
	//If both vectors are normalised, then when the normal is facing the light the output is 1.0, when facing away, -1.0.
	float LdotN			= dot(normalMap, sunDir);
	
	//Do the same, but for the reflection direction and normal.
	...
	/* Note:
	 * This tutorial was originally written for XNA 3.1,
	 * XNA 4 introduces support for FP10 render targets on the xbox, using the 'HDRBlendable' format.
	 * For render targets, this is a highly recommended format to use.
	 * However, this tutorial continues to use RGBM for render targets.
	 */
...
technique Character
{
   pass
   {
		VertexShader = compile vs_3_0 CharacterVS();
		PixelShader = compile ps_3_0 CharacterPS();
   }
}
