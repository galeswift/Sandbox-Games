...
void VelocityParticleVS_GpuTex3D(
				...
				uniform bool	useUserPositionOffset)
{
	...
	float3 worldPoint = mul(positionOut,worldMatrix).xyz;
	
	float3 direction = (1-any(velocityRotation.xyz)) * worldSpaceYAxis + velocityRotation.xyz;
	
	float3 viewDir = normalize(worldPoint - viewPoint);
	float len = length(velocityRotation.xyz) - abs(dot(viewDir,velocityRotation.xyz));
	
	float3 y_axis = normalize(cross(viewDir,direction));
	float3 x_axis = cross(viewDir,y_axis);
	
	float scale = velocityScale.x + velocityRotation.w * velocityScale.y;	
	x_axis *= (1+len * scale);
	
	//find the corner position
	float3 offset = x_axis * position.y + y_axis * position.z;
	
	//generate the complete vertex position
	positionOut.xyz += offset * size;
...
technique DrawVelocityBillboardParticles_GpuTex3D
{
	pass 
	{
		VertexShader	= compile vs_3_0 VelocityParticleVS_GpuTex3D(false,false);	PixelShader		= compile ps_3_0 ParticlePS(); 
	}
}

technique DrawVelocityBillboardParticlesColour_GpuTex3D
{
	pass 
	{
		VertexShader	= compile vs_3_0 VelocityParticleVS_GpuTex3D(true,false);		PixelShader		= compile ps_3_0 ParticlePS();
	}
}

//user variants

technique DrawVelocityBillboardParticles_GpuTex3D_UserOffset
{
	pass 
	{
		VertexShader	= compile vs_3_0 VelocityParticleVS_GpuTex3D(false,true);	PixelShader		= compile ps_3_0 ParticlePS(); 
	}
}

technique DrawVelocityBillboardParticlesColour_GpuTex3D_UserOffset
{
	pass 
	{
		VertexShader	= compile vs_3_0 VelocityParticleVS_GpuTex3D(true,true);		PixelShader		= compile ps_3_0 ParticlePS();
	}
}
...
technique DrawVelocityBillboardParticles_BillboardCpu3D
{
	pass 
	{ 
		VertexShader	= compile vs_2_0 VelocityParticleVS_Cpu3D(false); 
		PixelShader		= compile ps_2_0 ParticlePS(); 
	}
}


technique DrawVelocityBillboardParticlesColour_BillboardCpu3D
{
	pass 
	{ 
		VertexShader	= compile vs_2_0 VelocityParticleVS_Cpu3D(true); 
		PixelShader		= compile ps_2_0 ParticlePS(); 
	}
}
