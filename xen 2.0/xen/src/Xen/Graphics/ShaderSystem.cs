...
namespace Xen.Graphics
{
	...
	public interface IShaderGlobals
	{
		...
		/// <param name="name">name of the value (case sensitive)</param>
		/// <param name="value">value to assign the shader global</param>
		void SetShaderGlobal(string name, Microsoft.Xna.Framework.Graphics.Texture3D value);
		/// <summary>
		...
		/// <param name="value">output value to be assigned the shader global value (if it exists)</param>
		/// <remarks>True if the value exists, and has been ouput</remarks>
		bool GetShaderGlobal(string name, out Microsoft.Xna.Framework.Graphics.Texture3D value);
		/// <summary>
	...
	[System.Diagnostics.DebuggerStepThrough]
#endif
	sealed class ShaderSystemState : ShaderSystemBase, IShaderGlobals
	{
		...
		public override sealed int GetGlobalUniqueID<T>(string name)
		{
			...
			if (type == typeof(Microsoft.Xna.Framework.Graphics.Texture3D))
				return GetIndexTexture(ref texture3DGlobals, name, texture3DGlobalLookup);
		...
		public override sealed Microsoft.Xna.Framework.Graphics.Texture3D GetGlobalTexture3D(int uid)
		{
#if DEBUG
		...
		{
			int i = GetIndexTexture(ref texture3DGlobals, name, texture3DGlobalLookup);
...
#endif
			effect.Effect = new Effect(this.graphics, decompressed);
		...
		public override bool SetBlendMatricesDirect(EffectParameter param, ref int changeIndex)
		{
		...
		sealed class EffectParamMapper
		{
			...
			sealed class ParamMapping
			{
				private readonly MatrixMapping[] map;
				private readonly EffectParameter blendMatrices;
				private int blendChangedIndex;

				public ParamMapping(MatrixMapping[] map, EffectParameter blendMatrices)
				{
			...
			public bool ApplyMapping(Effect effect, ShaderExtension extension)
			{
				ParamMapping mapping;
				if (!effectMap.TryGetValue(effect, out mapping))
				{
					EffectParameter blendParam = null;
